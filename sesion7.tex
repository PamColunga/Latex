...
\usepackage{hyperref}
\usepackage{epstopdf}
%este ultimo paquete es solo para latex
...
% Son cosas que todavia no se numeran, osea lo que va en numeros romanos al inicio de los libros
\chapter{Prólogo}

En el futuro incluiremos un prólogo en esta sección.

\mainmatter
% A partir de aqui ya empieza la numeración
...
%scale 0.5 ajusta la imagen a la mitad de su tamaño, no modifica el archivo
\caption{Gráfica de $sen \left( x \right) +{{\rm e}^{\cos \left( {{\rm e}^{x}} \right) }}$}\label{cap1f1}
\end{figure}
%las imagenes hay que ponerlas mismo donde tenemos nuestro archivo.TEX
...
\includegraphics[scale=0.5]{graficasMapleplot2d4}
\end{figure}
